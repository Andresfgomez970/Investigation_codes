...
*/  
int DefineConstantKernel(Kernel_of_Function **kernel, int n_functions){
...
// This test initialize all function for each dimension to be mapped. 
int InitConstantKernelInfo(Kernel_Info *kernel){
  ...
  // When no params are given, as for a constant kernel of functions 
  //  the dimension on the parameter space of each function is chosen to be 1.
  ...
  DefineConstantKernel(&(kernel -> kernel_fuction), kernel -> n_functions);
  ...
  for(int i = 0; i < kernel.n_functions; i++){
    ...
    */
    ...
    GeneratePointFromInputFunction(kernel.function_param[i], &point[dim_mapped], kernel.kernel_fuction[i].f,
                                   kernel.val_0[i], kernel.val_f[i], kernel.dim_of_val[i]);
...
int GenerateNPoints1DNormal(void){
  double point[1];
  double param[2];
  double var_0[1];
  double var_f[1];
